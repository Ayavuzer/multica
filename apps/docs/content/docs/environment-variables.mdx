...
| Variable | Default | Description |
|---|---|---|
| `MULTICA_MEMORY_REVIEW_AGENT_ALLOWLIST` | ARIA + Gemini IDs | Comma-separated agent IDs allowed to approve or reject eligible memory entries as independent reviewers. Self-review is always denied; owner/admin review remains allowed. |

Leave this unset unless you need to replace the default hosted-workspace reviewer set. The server still enforces scope, guardrail, source issue, review note, audit, and rate-limit checks for agent reviewers.
