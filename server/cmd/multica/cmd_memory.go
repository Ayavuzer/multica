...
var memoryRevokeCmd = &cobra.Command{
	Use:   "revoke <id>",
	Short: "Revoke an approved memory",
	Args:  exactArgs(1),
	RunE:  runMemoryRevoke,
}
...
func init() {
	...
	memoryCmd.AddCommand(memoryRevokeCmd)

	for _, cmd := range []*cobra.Command{memoryListCmd, memorySearchCmd} {
		cmd.Flags().String("status", "", "Filter by status: pending, approved, rejected, or revoked")
	...
	memoryApproveCmd.Flags().String("review-note", "", "Review note (canonical)")
	memoryApproveCmd.Flags().String("source-issue", "", "Review source issue ID or identifier")
	memoryApproveCmd.Flags().String("source-comment", "", "Review source comment ID")
	memoryApproveCmd.Flags().String("output", "json", "Output format: table or json")

	memoryRejectCmd.Flags().String("note", "", "Review note")
	memoryRejectCmd.Flags().String("review-note", "", "Review note (canonical)")
	memoryRejectCmd.Flags().String("source-issue", "", "Review source issue ID or identifier")
	memoryRejectCmd.Flags().String("source-comment", "", "Review source comment ID")
	memoryRejectCmd.Flags().String("output", "json", "Output format: table or json")

	memoryRevokeCmd.Flags().String("note", "", "Review note")
	memoryRevokeCmd.Flags().String("review-note", "", "Review note (canonical)")
	memoryRevokeCmd.Flags().String("source-issue", "", "Review source issue ID or identifier")
	memoryRevokeCmd.Flags().String("source-comment", "", "Review source comment ID")
	memoryRevokeCmd.Flags().String("output", "json", "Output format: table or json")
}
...
func runMemoryRevoke(cmd *cobra.Command, args []string) error {
	return runMemoryReview(cmd, args[0], "revoke")
}
...
func runMemoryReview(cmd *cobra.Command, id, action string) error {
	...
	reviewNote, _ := cmd.Flags().GetString("review-note")
	body := map[string]any{}
	if strings.TrimSpace(reviewNote) != "" {
		body["review_note"] = strings.TrimSpace(reviewNote)
	} else if strings.TrimSpace(note) != "" {
		body["review_note"] = strings.TrimSpace(note)
		body["note"] = strings.TrimSpace(note)
	}
	if sourceIssue, _ := cmd.Flags().GetString("source-issue"); strings.TrimSpace(sourceIssue) != "" {
		body["source_issue"] = strings.TrimSpace(sourceIssue)
	}
	if sourceComment, _ := cmd.Flags().GetString("source-comment"); strings.TrimSpace(sourceComment) != "" {
		body["source_comment"] = strings.TrimSpace(sourceComment)
	}
